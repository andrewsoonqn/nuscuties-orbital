...
public partial class PassiveOngoing : Control
{
	...
	public override void _Ready()
	{
		...
		_timeSpentLabel.Text = Mathf.Floor(_timeSpent / 60.0).ToString("F0");
		_totalTimeLabel.Text = $"out of {_totalTime:F0} minutes";
	...
	public override void _Process(double delta)
	{
		_timeSpent += delta;
		_timeSpentLabel.Text = Mathf.Floor(_timeSpent / 60.0).ToString("F0");
		_timeProgressBar.Ratio = (_timeSpent / 60.0) / _totalTime;

		_expAccumulated = (int)((_timeSpent / 60.0) * 50);
		_expAccumulatedLabel.Text = $"{_expAccumulated:F0} EXP";

		_coinsAccumulated = (int)((_timeSpent / 60.0) * 5);
		if (_coinsAccumulatedLabel != null)
		{
			_coinsAccumulatedLabel.Text = $"{_coinsAccumulated:F0} Coins";
		}
		
		if ((_timeSpent / 60.0) >= _totalTime)
		{
			OnEnd("Finished");
		}
	}
